...
/// <summary>
///普通UI循环列表，prefabSource选定子物体,cachePlace设置缓存池位置(一般在子物体content同级建一隐藏的空物体)(暂未开发选用资源路径)
/// 1. 给 ITEM_MAX_COUNT 赋值(必选)
/// 2. 设置 m_pCallback 回调(必选)
/// 3. RefillCells() 初始绘制(必选)
/// 4.RefreshCells() 刷新现在显示的条目(非必选)
/// 5.m_pRemoveCallback 回调(非必选，当需要将移除对象初始化时，可以用)
/// 
/// 
/// 聊天模式：列表数据从上往下，数据超过屏幕时候底部为最新消息
/// 0.设置滑动模式为Clamped
/// 1. 设置 m_pCallback 回调(必选 可在Awake)
/// 2.给 ITEM_MAX_COUNT 赋值(必选) 数据改变/初始化时re
/// 4. RefillCellsFromEnd()绘制 (必选)
/// </summary>
/// 
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        [Tooltip("循环item缓存池节点(隐藏)")]
        public GameObject cachePlace;
        ...
        public delegate void RemoveOneCallback(GameObject go);   //移除一个物件的回调
        public RemoveOneCallback m_pRemoveCallback = null;

        private Vector2 m_vLastDragPos;
        public Action<Vector2> m_pDragCallback = null;  // scroll rect的drag操作回调
        public Action m_BeginDragCallback = null;//开始滑动回调
        ...
        public virtual bool IsInViewport(RectTransform trans) { return false; }
        //==========LoopScrollRect==========
        ...
        private void SetData(GameObject go, int index)
        {
            //go.GetComponentInChildren<Text>().text = m_pFakeValue[index].ToString();
            if (null != m_pCallback)
            {
        ...
        private GameObject GetObjectFromPool()
        {
            ...
            {
                result = Instantiate(prefabSource);
                return result;
            }
            result = cachePool[cachePool.Count - 1];
            if (cachePool.Remove(result))
            {
                return result;
            }
            return null;
        ...
        private void ReturnObjectToPool(GameObject go)
        {
            ...
            cachePool.Add(go);
            go.transform.SetParent(cachePlace.transform);
            go.SetActive(false);
            if (m_pRemoveCallback != null)
            {
                m_pRemoveCallback(go);
            }
        }
        ...
        public void RefreshCells()
        {
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                ...
                {
                    if (itemTypeEnd < ITEM_MAX_COUNT)
                    {
                        //dataSource.ProvideData(content.GetChild(i), itemTypeEnd);
                        SetData(content.GetChild(i).gameObject, itemTypeEnd);
                        itemTypeEnd++;
                    }
                    else
                    {
        ...
        public void RefillCellsFromEnd(int offset = 0)
        {
            ...
            if (!Application.isPlaying || ITEM_MAX_COUNT < 0 || contentConstraintCount > 1 || prefabSource == null)
                return;

            StopMovement();
            itemTypeEnd = reverseDirection ? offset : ITEM_MAX_COUNT - offset;
        ...
        public void RefillCells(int offset = 0)
        {
            ...
            StopMovement();
            itemTypeStart = reverseDirection ? ITEM_MAX_COUNT - offset : offset;
            itemTypeEnd = itemTypeStart;
        ...
        protected float NewItemAtStart()
        {
            ...
            if (ITEM_MAX_COUNT >= 0 && itemTypeStart - contentConstraintCount < 0)
            {
                return 0;
            }
        ...
        protected float DeleteItemAtStart()
        {
            ...
            if (((m_Dragging || m_Velocity != Vector2.zero) && ITEM_MAX_COUNT >= 0 && itemTypeEnd >= ITEM_MAX_COUNT - 1)
        ...
        {
            if (ITEM_MAX_COUNT >= 0 && itemTypeEnd >= ITEM_MAX_COUNT)
            {
                return 0;
            }
            float size = 0;
        ...
        {
            if (((m_Dragging || m_Velocity != Vector2.zero) && ITEM_MAX_COUNT >= 0 && itemTypeStart < contentConstraintCount)
                || content.childCount == 0)
            {
                return 0;
            }
        ...
        private RectTransform InstantiateNextItem(int itemIdx)
        {
            ...
            SetData(nextItem.gameObject, itemIdx);
            return nextItem;
        }
        //==========LoopScrollRect==========
        ...
        public virtual void OnBeginDrag(PointerEventData eventData)
        {
            ...
            m_Dragging = true;
            m_vLastDragPos = eventData.position;
        }
        ...
        public virtual void OnEndDrag(PointerEventData eventData)
        {
            ...
            if (null != m_pDragCallback)
            {
                Vector2 delta = eventData.position - m_vLastDragPos;
                //ClientLog.Log(delta);
                m_pDragCallback(delta);
            }
        }
        ...
        private void UpdateScrollbars(Vector2 offset)
        {
            ...
            {
                //==========LoopScrollRect==========
                if (m_ContentBounds.size.x > 0 && ITEM_MAX_COUNT > 0)
                {
                    m_HorizontalScrollbar.size = Mathf.Clamp01((m_ViewBounds.size.x - Mathf.Abs(offset.x)) / m_ContentBounds.size.x * (itemTypeEnd - itemTypeStart) / ITEM_MAX_COUNT);
                }
                //==========LoopScrollRect==========
                else
            ...
            {
                //==========LoopScrollRect==========
                if (m_ContentBounds.size.y > 0 && ITEM_MAX_COUNT > 0)
                {
                    m_VerticalScrollbar.size = Mathf.Clamp01((m_ViewBounds.size.y - Mathf.Abs(offset.y)) / m_ContentBounds.size.y * (itemTypeEnd - itemTypeStart) / ITEM_MAX_COUNT);
                }
                //==========LoopScrollRect==========
                else
        ...
        /// <summary>
        /// 手动设置滚到指定个位置. value=定位偏移/(内容总高度-view的高度）(0~1), 有个问题m_MovementType 是Elastic 会乱滚， 建议设置成clamped模式
        /// </summary>
        ...
        protected Bounds GetBounds(RectTransform trans = null)
        {
            if (null == trans)
            {
                trans = m_Content;
        ...
        private Vector2 CalculateOffset(Vector2 delta)
        {
            ...
            if (m_Horizontal)
            {
                min.x += delta.x;
                max.x += delta.x;
                if (!Mathf.Approximately(max.x, m_ViewBounds.max.x) && min.x > m_ViewBounds.min.x)
                    offset.x = m_ViewBounds.min.x - min.x;
                else if (!Mathf.Approximately(min.x, m_ViewBounds.min.x) && max.x < m_ViewBounds.max.x)
            ...
            if (m_Vertical)
            {
                min.y += delta.y;
                max.y += delta.y;
                if (!Mathf.Approximately(max.y, m_ViewBounds.max.y) && max.y < m_ViewBounds.max.y)
                    offset.y = m_ViewBounds.max.y - max.y;
                else if (!Mathf.Approximately(min.y, m_ViewBounds.min.y) && min.y > m_ViewBounds.min.y)
        ...
        protected override void Reset()
        {
            ...
            if (cachePlace == null)
            {
                Transform cachePos = transform.Find("cachePool");
                GameObject cachePool;
                if (cachePos != null)
                {
                    cachePool = cachePos.gameObject;
                }
                else
                {
                    cachePool = new GameObject();
                    cachePool.name = "cachePool";
                    cachePool.SetActive(false);
                    cachePool.transform.parent = transform;
                    cachePool.transform.localScale = Vector3.one;
                    cachePool.transform.localPosition = Vector3.zero;
                }
                cachePlace = cachePool;
            ...
            if (viewport == null)
            {
                var trans = transform.Find("Viewport");
                if (trans)
                {
                    viewport = trans.GetComponent<RectTransform>();

                    if (content == null)
                    {
                        var contentObj = transform.Find("Viewport/Content");
                        if (contentObj)
                        {
